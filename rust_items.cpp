#include <cstddef>
#include <cstring>
#include "rust_items.h"

struct RustItemEntry {
    int id;
    const char* name;
};

static const RustItemEntry rust_item_table[] = {
    ...
    { -2016974826, "Lunar Wall Frame Floral" },
    ...
    { -1899491405, "Glue" },
    { -1884328185, "Scrap Transport Helicopter" },
    ...
    { -450890885, "Lunar Wall Frame Swirling" },
    ...
    { 62577426, "Photograph" },
    { 63265850, "High External Frontier Wall" },
    ...
    { 363163265, "Hose Tool" },
    { 368008432, "Crafting Quality Tea" },
    ...
    { 472505338, "Medieval Assault Rifle" },
    ...
    { 1115193056, "Lunar Wall Frame Inlay" },
    ...
    { 2137338174, "High External Frontier Gate" }
};

const char* RustItem_GetName(int id) {
    unsigned int count = sizeof(rust_item_table) / sizeof(rust_item_table[0]);
    for (unsigned int i = 0; i < count; ++i) {
        if (rust_item_table[i].id == id) {
            return rust_item_table[i].name;
        }
    }
    return "Unknown Item";
}
...
int RustItem_GetID(const char* name) {
    unsigned int count = sizeof(rust_item_table) / sizeof(rust_item_table[0]);
    for (unsigned int i = 0; i < count; ++i) {
        if (strcmp(rust_item_table[i].name, name) == 0) {
            return rust_item_table[i].id;
        }
    }
    return 0;
}
...
int RustItem_GetCount() {
    return sizeof(rust_item_table) / sizeof(rust_item_table[0]);
}
